...
class AnnonceVenteViewSet(viewsets.ModelViewSet):
    ...
    def toutes_annonces(self, request):
...
class PropositionAchatViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['get'], url_path='toutes')
    def toutes_propositions(self, request):
        propositions = self.get_queryset()
        serializer = self.get_serializer(propositions, many=True)
        return Response(serializer.data)

    @action(detail=False, methods=['get'], url_path='mes-propositions')
